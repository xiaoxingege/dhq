...
<div class="map_wrap">
  ...
  <div v-bind:class="{'chart_bottom':!isEnlarge,'chart_bottom_enlarge':isEnlarge}">
    <div class="clearfix playback">
      ...
      <div class="play_line" ref="playLine" :style="{left:playBackIndex*35+playLineLeft+'px'}" v-if="(!isEnlarge || isPlaybackShow) && playBackIndex>=0"></div>
      <!--div v-for="date of playBackDateShow" class="perday" v-if="!isEnlarge || isPlaybackShow">{{date}}</div-->
      <div v-for="(time,index) of timeList.slice(1)" :class="playBackIndex<=index?'perday disable_time':'perday'" v-if="!isEnlarge || isPlaybackShow">{{Number(time.substring(0,2))+":"+time.substring(2)}}</div>
...
export default {
  ...
  data() {
    return {
      ...
      updateTimePid: null,
      currentTime: '',
      // playLineIndex: 0, // 回放index
      playBackIndex: 0, // 回放index
  ...
  methods: {
    ...
    updateDatetime: function() {
      this.$store.dispatch('stockMap/queryCurTimeItem').then(() => {
        ...
        this.playBackTime = this.$store.state.stockMap.curTimeItem;
        // this.playBackIndex = this.datetimeIndex = this.timeList.indexOf(this.playBackTime);
      })
    },
