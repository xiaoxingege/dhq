...
<template>
  <div class="map_wrap">
      ...
      <div class="chart" ref="treemap" v-on:mousemove="move($event,$index)" :style="{height:mapHeight+'px'}"></div>
      <!--<div class="hover-wrapper"> <h3>银行 - 银行</h3> <table><tbody> <tr class="hovered" style="background-color:#32844e;"><td class="ticker" width="98">中信银行</td> <td ></td> <td class="price">&#45;&#45;</td> <td class="change">-3.41%</td> </tr> <tr class="hovered is-description" style="background-color:#32844e;height:16px;"> <td colspan="4" class="description"></td> </tr> <tr> <td class="ticker">工商银行</td> <td></td> <td class="price">&#45;&#45;</td> <td class="change" style="color:#35764e;">-2.81%</td> </tr> <tr> <td class="ticker">农业银行</td> <td></td> <td class="price">&#45;&#45;</td> <td class="change" style="color:#32844e;">-3.69%</td> </tr> <tr> <td class="ticker">中国银行</td> <td></td> <td class="price">&#45;&#45;</td> <td class="change" style="color:#3a614f;">-1.73%</td> </tr> </tbody></table></div>-->
      <div class="chart_bottom clearfix">
          <div class="clearfix playback">
              ...
              <div class="play_line" ref="playLine" :style="{left:playBackIndex*37+48+'px'}"></div>
       ...
       this.$store.dispatch('stockMap/queryRangeByCode', { code: this.rangeCode })
                    .then(() => {
                      this.chart.setOption({
                        tooltip: {
                          ...
                          formatter: function (params) {
                            ...
                            let bgColor = ''
                            let titleStockName = ''
                            let titleStockCondtion = ''
                            if (info.treePathInfo.length <= 2) {
                              return
                            }
                            if (info.data.children.length > 0) {
                              stockList = info.data.children
                              bgColor = stockList[0].itemStyle.normal.color
                              titleStockName = stockList[0].name
                              titleStockCondtion = stockList[0].perf
                              _this.id = info.data.id
                            } else {
                              _this.mapData.forEach(function (industry) {
                                industry.children.forEach(function (lel2) {
                                  if (lel2.id === info.data.parent) {
                                    stockList = lel2.children
                                    bgColor = info.color
                                    titleStockName = info.data.name
                                    titleStockCondtion = info.data.perf
                                    _this.id = info.data.parent
                                    return
                                  }
                                })
                              })
                            }
                            let hoverHtml = '<div class="hover-wrapper"><h3>'
                            hoverHtml += (info.treePathInfo[1].name + '-' + info.treePathInfo[2].name + '</h3> <table><tbody>')
                            hoverHtml += '<tr class="hovered" style="background-color:' + bgColor + ';"><td class="ticker">' + titleStockName + '</td> <td ></td> <td class="price">--</td> <td class="change">' + titleStockCondtion + '</td> </tr> <tr class="hovered is-description" style="background-color:' + bgColor + ';height:16px;"> <td colspan="4" class="description"></td> </tr>'
                            for (var i = 0; i < stockList.length; i++) {
                              hoverHtml += '<tr><td class="ticker">' + stockList[i].name + '</td> <td></td> <td class="price">--</td> <td class="change" style="color:#35764e;">' + stockList[i].perf + '</td> </tr>'
                            }
                            hoverHtml += '</tbody></table></div>'*/
                            // return hoverHtml
                           // return '<div id="hover-wrapper"></div>'
                          }
                        },
                        series: [
                          {
                      ...
                      this.chart.on('mouseover', (params) => {
                        ...
                        let titleStockCondtion = ''
                        let stockId = ''
                        const titleName = params.treePathInfo[1].name
                        const titleNameLel2 = params.treePathInfo[2].name
                        if (params.data.children.length > 0) {
                          stockList = params.data.children
                          bgColor = stockList[0].itemStyle.normal.color
                          titleStockName = stockList[0].name
                          titleStockCondtion = stockList[0].perf
                          stockId = params.data.id
                        } else {
                          _this.mapData.forEach(function (industry) {
                            industry.children.forEach(function (lel2) {
                              if (lel2.id === params.data.parent) {
                                stockList = lel2.children
                                bgColor = params.color
                                titleStockName = params.data.name
                                titleStockCondtion = params.data.perf
                                stockId = params.data.parent
                                return
                              }
                            })
                          })
                        }
                        const stockListLeft = _this.offsetX + 'px'
                        const stockListTop = _this.offsetY + 'px'
                       // console.log(stockListLeft, stockListTop)
                        new Vue({
                          template: '<div><StockList :condition="condition" :indexCode="rangeCode" :isUnit="isUnit" :stockList="stockList" :bgColor="bgColor" :stockId="stockId" :titleStockName="titleStockName" :titleStockCondtion="titleStockCondtion" :titleName="titleName" :titleNameLel2="titleNameLel2" :stockListLeft="stockListLeft" :stockListTop="stockListTop"/></div>',
                          el: '#hover-wrapper',
                          data: {
                            stockList: stockList,
                            bgColor: bgColor,
                            stockId: stockId,
                            titleStockName: titleStockName,
                            titleStockCondtion: titleStockCondtion,
                            titleName: titleName,
                            titleNameLel2: titleNameLel2,
                            stockListLeft: stockListLeft,
                            stockListTop: stockListTop,
                      ...
                      this.$store.dispatch('stockMap/updateData', { isContinue: this.isContinue, condition: this.condition, code: this.rangeCode }).then(() => {
                        ...
                        this.$store.dispatch('stockMap/calendarsData', {}).then(() => {
