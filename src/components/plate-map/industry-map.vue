...
export default {
  ...
  data() {
    return {
      ...
      colors: {
        /* 板块 */
        'indu_market.tech_index': colorsList1.slice().reverse(), // 热度指数 灰-红
        'chg_pct': colorsList1.slice().reverse(), // 涨跌幅 绿-红
        'indu_market.rise_speed_3min':colorsList1.slice().reverse(), // 三分钟涨速
        ...
        'mkt_idx.rising_rate': colorsList1.slice().reverse(), // 三分钟涨速
      ...
      rangeValues: {
        ...
        /* 板块 */
        'indu_market.tech_index': valueRangeRD, // 热度指数
        'chg_pct': valueRange1d, // 涨跌幅
        'indu_market.rise_speed_3min' : this.fmtraneValue(valueRange1d, 0.125), // 三分钟涨速
      ...
      isUnit: {
        ...
        /* 板块 */
        'indu_market.tech_index': '', // 热度指数 绿-红
        'chg_pct': '%', // 个股涨跌幅 绿-红
        'indu_market.rise_speed_3min':'', // 三分钟涨速 
  ...
  computed: {
    mapData: function() {
      const map = [].concat(this.$store.state.plateMap.industryData)
    ...
    industryValue: function() {
      ...
      map.forEach(function(stock) {
        if (industryValue) {
          ...
          stock.itemStyle = {
            normal: {
              color: _this.showColor(_this.colors[_this.conditionIndustry], _this.rangeValues[_this.conditionIndustry], stock.perf) || '#2f323d'
            }
          }
        } else {
          stock.perfText = '--'
  ...
  methods: {
    initMap: function() {
      ...
      let p1 = new Promise((resolve, reject) => {
        this.$store.dispatch('plateMap/queryIndustry').then(() => {
          this.initOption(this.mapData);
          resolve();
        })
      });
      let p2 = new Promise((resolve, reject) => {
        this.$store.dispatch('plateMap/queryIndustryValue', {
          isContinue: this.isContinue,
          condition: this.conditionIndustry
        }).then(() => {
          resolve();
        })
      });
    ...
    updateStockMap: function() {
      ...
      let p2 = new Promise((resolve, reject) => {
        this.$store.dispatch('plateMap/queryIndustryStockValue', {
          isContinue: this.isContinue,
          condition: this.conditionStockI,
          industryCode: this.industryCode
        }).then(() => {
          resolve();
        })
    ...
    updateData: function() {
      ...
      this.$store.dispatch('plateMap/queryIndustryValue', {
        ...
        // 如果条件已改变则不再执行回调方法
        if (condition !== this.conditionIndustry) {
    ...
    updateStockData: function() {
      this.getLegendColor(this.conditionStockI)
      this.$store.dispatch('plateMap/queryIndustryStockValue', {
        isContinue: this.isContinue,
        condition: this.conditionStockI,
        industryCode: this.industryCode
        ...
        // 如果条件已改变则不再执行回调方法
        if (condition !== this.conditionStockI) {
          console.info('invalide callback and do nothing');
          return;
        }
        this.updateMapData(this.industryStockValue)
    ...
    clickPlate: function(type, id) {
      this.timeoutID = setTimeout(() => {
        if (type === 'click') {
          ...
          this.$emit('passConditionStockI', this.conditionIndustry)
          this.showHover = false
          this.industryCode = id
          this.updateStockMap()
        } else if (type === 'dblclick') {
          window.open(ctx + '/industry/' + id)
        }
      }, 250);
    },
    ...
    queryPlaybackData: function(date, time) {
      this.$store.dispatch('plateMap/updateIndustryDataByTime', {
        time: date + time
      }).then(() => {
