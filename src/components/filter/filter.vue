      ...
      <div class="fundPool fl">
        ...
        <ul class="fundPoolList">
          ...
          <li v-if="lsfoundPoolList.length === 0">
            <div class="defaultTxt tc"><p>每个基金池最多可添加50只基金，</p><p>保存基金池后，可快速创建组合</p></div>
          </li>
        </ul>
      </div>
      <!-- 临时基金池 end-->
        ...
        <table class="table tc">
          ...
          <tbody>
            <tr v-for='(item,index) in foundPoolListData.foundPoolList'>
              ...
              <td v-if='typeIndex === 1 || typeIndex === 2'>
                <p class="hyname tc" :title='item.hyName'>{{item.hyName | isNull}}</p>
              </td>
        ...
        foundPoolListData: function (state) {
          ...
          return {
            foundPoolList: list.map((fund, index) => {
              const tempFund = { ...fund,
                inTempPools: this.isInTempPoollist(fund.innerCode)
              }
              return tempFund
            })
          }
      ...
      _saveFundPool (fundCodes) {
        ...
        return fetch(url, {
          ...
          if (result.errCode === 0) {
            this.dialogShow = false
            this.$router.push({
              path: ctx + '/foundpooldetail/' + result.data + '?orgCode=' + this.orgCode
            })
