...
<div class="dime-tech">
  <div>
    <div class="techline-title">
      {{techFace.technicalTitle}}
      <span class="assess1" v-for='(techDetail,index) of techFace.moonDetailVoMap' :class="checkStatus(techDetail.colourCode)">{{techDetail.flagResume==null?'--':techDetail.flagResume}}</span>
    </div>
    <div class="techline-title2">{{techFace.technicalSummary}}</div>
...
export default ({
  ...
  methods: {
    init() {
      const klineData = [].concat(this.techFace.rangeDatas)
      ...
      klineData.forEach((item, index) => {
        ...
        console.log(this.techFace.valueRange)
        if (this.techFace.valueRange === range) {
          var newValue3 = {}
          var newValue20 = {}
