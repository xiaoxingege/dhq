...
<div class="smartPoolListDetails">
  ...
  <table class="table tc">
    ...
    <tr v-for="(item,index) in smartPoolListDetails">
      ...
      <td>
        <div>{{item.fundTypeName}}</div>
      </td>
      <td>
        <div>{{item.fundScale}}</div>
      </td>
      <td>
        <div v-z3-updowncolor="item.chgPct">{{item.chgPct | filterNum('%')}}</div>
      </td>
      <td>
      ...
      <td>
        <div>{{item.sgflRate}}</div>
      </td>
      <td>
        <div>{{item.shflRate}}</div>
      </td>
      <td>
        <div>{{item.sellDay}}</div>
      </td>
      <td>
        <div>{{item.fundYieldYearRank}}</div>
      </td>
    </tr>
  </table>
...
export default {
  ...
  methods: {
    getdate () {
      ...
      this.$store.dispatch('getSmartPoolListDetailsTop', {
    ...
    copySmartPool () {
      ...
      return fetch(url, {
        ...
        if (result.errCode === 0) {
          this.show = false
          this.$router.push({
            path: ctx + '/foundpooldetail/' + result.data
          })
...
.seletetime {
    ...
    .downicon {
        ...
        border-bottom: 6px solid #2f94ee;
        vertical-align: 2px;
    }
    ul {
        width: 100%;
