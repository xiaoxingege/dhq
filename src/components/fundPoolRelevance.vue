...
.fundPoolRelevance .line {
    position: absolute;
    width: 7px;
    height: 100%;
    background: linear-gradient(to bottom, rgb(255, 255, 255), rgb(35, 136, 218));
    left: 22px;
}
.fundPoolRelevance .line i {
    position: absolute;
    left: -15px;
    font-size: 14px;
}
.fundPoolRelevance .line i:nth-child(1) {
    top: 0;
}
.fundPoolRelevance .line i:nth-child(2) {
    bottom: 0;
}
.tablebox {
    position: relative;
      ...
      <table>
        <tr v-for="(item,index) in relevancedata.relevance">
          <td v-for="(item2,index2) in item">
            <div v-on:mouseenter="dataDetails($event)" v-on:mouseleave="hiddenDetail($event)" :dataindexw="index" :dataindexi="index2"><i :style="{'opacity':bgcolor(item2)}"></i><span>{{item2 | floatfn}}</span></div>
          </td>
        </tr>
      </table>
    </div>
...
export default {
  ...
  data () {
    return {
      colorarr: ['yellow', 'yellow', 'yellow', ['#7cbe39', '#c4e0a9'],
        ['#cc5159', '#cea6a8'],
        ['#3a71a3', '#8ea2b4'],
        ['#cf4b8d', '#ddb3c8'],
        ['#c96b37', '#deb29a'], 'yellow', 'yellow', 'yellow', ['#7e58af', '#bcb1c9'],
        ['#35a392', '#9ac0ba']
      ],
      creatediv: '',
      colorjson: {}
