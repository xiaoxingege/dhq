...
<div class="playback_con">
  ...
  <div v-for="(time,index) of timeList.slice(1)" :class="datetimeIndex<=index?'perday disable_time':'perday'" @click="index <= datetimeIndex && jumpToPlay(index+1)" v-if="!isFullScreen || isplaybackShow">{{Number(time.substring(0,2))+":"+time.substring(2)}}</div>
