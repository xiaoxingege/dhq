...
.exchrHld .main ul li span,
.fundFeature .main ul li span {
...
.secondLine span:last-child,
.thirdLine span:last-child {
    border-right: none;
    width: 50px;
}
.rectY span {
...
.rectTime span:first-child,
.rectTime span:last-child {
    width: 37px;
    height: 24px;
    color: #fff;
    background: #CCCCCC;
}
.rectTime span.active,
.rectTime span.active {
    width: 37px;
    height: 24px;
    color: #fff;
...
export default {
  ...
  mounted() {
    this.$store.dispatch('fundRecord/getFeatureData', {
      innerCode: '000001.CW'
    }).then(() => {

    })
    this.$store.dispatch('fundRecord/getExchrHldData', {
      innerCode: '000001.CW'
    }).then(() => {
