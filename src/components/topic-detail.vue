...
<div class="topic-detail">
  <div class="topic-head clearfix">
    ...
    <div class="topic-time fl">
      ...
      </span><span class="time-num4">持平股票</span><span class="time-num2">{{detail.topicMarket===null || detail.topicMarket.stkEqualNum==null?'--':detail.topicMarket.stkEqualNum}}
      ...
      <div class="main-left box-flex-1">
        ...
        <div class="left-con1 display-box">
          ...
          <div class="con1-event box-flex-1">
            <strong>驱动事件：</strong>
            <router-link :to="{name:'detailPages',params:{id : detail.eventId, detailType:'news'}}" target="_blank" v-if="detail.drivenEvent!=null">
              <span class="tip-1 tip-event">{{checkNull(detail.drivenEvent)}}</span>
            </router-link>
            <span class="tip-1 tip-event" v-else>--</span>
          </div>
        </div>
...
export default {
  ...
  computed: {
    ...mapState({
      ...
      topicTechAndInfoData: state => {
        const topicData = state.topic.topicTechAndInfo
        const indexValue = []
        const maValue = []
        const tradeDate = []
        const pointData = []
  ...
  methods: {
    ...
    renderMa5() {
      this.$store.dispatch('topic/queryTopicTechAndInfo', {
          topicCode: this.topicCode,
    ...
    heatOrPublicChange(type) {
      ...
      if (type === 'rise') {
        ...
        newSpan.id = 'expres';
        newSpan.innerHTML = '<sub>t</sub>)-1计算累计收益率曲线，其中r<sub>t</sub>代表第t日题材涨跌幅。';
    ...
    initInformatList() {
      this.$store.dispatch('topic/queryInformatList', { // 记录 记录记录记录
        topicCode: this.topicCode,
