...
export default {
  ...
  mounted() {
    ...
    this.$store.dispatch('backtestDetailH5/getStrategyData', {
      strategyId: this.strategyId
    }).then(() => {
      if (this.goldResult !== null && this.goldResult.stockStrategyId !== null && this.goldResult.stockStrategyId !== '') {
        this.$store.dispatch('backtestDetailH5/getStockStrategyIdData', {
          strategyId: this.goldResult.stockStrategyId
        }).then(() => {})
      }
      if (this.goldResult !== null && this.goldResult.stockPoolId !== null && this.goldResult.stockPoolId !== '') {
        this.$store.dispatch('backtestDetailH5/getStockPoolIdData', {
          poolId: this.goldResult.stockPoolId
        }).then(() => {})
      }
    })
    this.$store.dispatch('backtestDetailH5/getBacktestData', {
      backtestId: this.backtestId
    }).then(() => {})
    this.$store.dispatch('backtestDetailH5/getSyqxtData', {
      backtestId: this.backtestId
    }).then(() => {})
  }
}
