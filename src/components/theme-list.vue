    ...
    <li class="theme-bar-li box-flex-1" v-for="topic of topicList">
      <div class="li-nei">
        ...
        <div class="bar-txt clearfix">
          <strong>最新事件：</strong>
          <span class="txt-con">
          <router-link :to="{name:'detailPages',params:{id : topic.newsId, detailType:'news'}}" class="new-text"><span
                  class="event">{{checkNull(topic.summary)}}</span></router-link>
        ...
        <div class="li-bottom" v-if="topic.topicMarket==null">
          今日涨跌<a class="bottom-Market">--</a> 上涨股票
          <span class="bottom-Market2">--</span>下跌股票<span class="bottom-Market3">--</span>
        </div>
        <div class="li-bottom" v-else>
          今日涨跌<a class="bottom-Market" v-z3-updowncolor="topic.topicMarket.chngPct">{{topic.topicMarket==null || topic.topicMarket.chngPct==null?'--':changeTofixed(topic.topicMarket.chngPct)}}</a> 上涨股票
          <span class="red bottom-Market2">{{topic.topicMarket==null || topic.topicMarket.stkUpNum==null?'--':topic.topicMarket.stkUpNum}}</span>下跌股票<span class="green bottom-Market3">{{topic.topicMarket==null || topic.topicMarket.stkDownNum==null?'--':topic.topicMarket.stkDownNum}}</span>
        </div>
      </div>
    </li>
  </ul>
