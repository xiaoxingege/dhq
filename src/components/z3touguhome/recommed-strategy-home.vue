...
.recommend-strategy-title {
    height: 9.5%;
}
.recommend-strategy-title p {
    color: #4c8cca;
    height: 100%;
}
.more-strategy {
...
.recommend-strategy-chart-link {
    display: inline-block;
    width: 100%;
    height: 79.5%;
}
.recommend-strategy-chart {
    height: 100%;
}
...
<div class="recommend-strategy" :style="{width:RecommendStrategyWidth}">
  <div class="recommend-strategy-title clearfix">
    <p class="strategy-name">
      <router-link :to="{name:'goldStrategy',params:{strategyId:strategyId}}">{{recommendStrategyName}}</router-link>
    </p>
    <p class="more-strategy"><a v-on:click="moreGoldList">更多推荐策略</a></p>
  </div>
  <router-link :to="{name:'goldStrategy',params:{strategyId:strategyId}}" class="recommend-strategy-chart-link">
