...
<div class="box">
  <div class="ui-block-inner">
    ...
    <div class="ui-five" id="d1">
      ...
      <a href="http://itougu.jrj.com.cn/tips/439.jspa" class="link2" @click="grab"></a>
    </div>
...
export default {
  ...
  methods: {
      ...
      if (window.app.name === '{{appid}}') {
        ...
        if (this.loginStatus === 'no') {
          ...
          this.$store.dispatch('actZdfl/playLottery').then(() => {
            if (!this.err) {
              let prize = this.lotteryResult.prizePosition * 1
              if (prize === 0) {
                prize = 8
              }
              this.prize = prize
    ...
    showLotteryResult() {
      if (this.prize > 0) {
        if (this.prize === 8) {
          this.pop2Html = '<h3>遗憾了！<br />大奖与您擦肩而过</h3><p class="fz26 cl1">参与现金券抢购，</p><p class="fz20 cl1">可以获得更多抽奖机会。</p><p class="fz20 cl1">祝您投资愉快</p>'
        } else {
