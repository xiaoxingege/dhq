    ...
    actions: {
        ...
        getLiveInfo({ commit, state }, data) {
            return new Promise((resolve, reject) => {
                const url = `http://mapi.itougu.jrj.com.cn/xlive_poll/queryViewAndTalkForNew/${data.roomId}?onlyView=true`;
        ...
        getHistoryInfo({ commit, state }, data) {
            return new Promise((resolve, reject) => {
                const url = `http://mapi.itougu.jrj.com.cn/wireless/xlive/queryViewAndTalkForOld/${data.roomId}/${data.timeId}?onlyView=true`;
                return fetch(url).then((res) => {
                    return res.json()
                }).then((data) => {
                    if (data.retCode) {
